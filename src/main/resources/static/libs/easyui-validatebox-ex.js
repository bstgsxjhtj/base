$.extend($.fn.validatebox.defaults.rules, {
	CHS : {
		validator : function(value, param) {
			return /^[\u0391-\uFFE5]+$/.test(value);
		},
		message : '请输入汉字'
	},
	ZIP : {
		validator : function(value, param) {
			return /^[1-9]\d{5}$/.test(value);
		},
		message : '邮政编码不存在'
	},
	QQ : {
		validator : function(value, param) {
			return /^[1-9]\d{4,10}$/.test(value);
		},
		message : 'QQ号码不正确'
	},
	phone : {// 验证电话号码 
		validator : function(value) {
			return /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i
					.test(value);
		},
		message : '格式不正确,请使用下面格式:020-88888888'
	},
	mobile : {
		validator : function(value, param) {
			return /^1[3|4|5|8|9]\d{9}$/.test(value);
		},
		message : '手机号码不正确'
	},
	phoneAndMobile:{
		validator : function(value, param) {
			if(/^1[3|4|5|8|9]\d{9}$/.test(value) || 
					/^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i
							.test(value)){
				return true;
			} 
			return false;
		},
		message : '请输入手机或座机号码格式:020-88888888或13888888888'
	},
	loginName : {
		validator : function(value, param) {
			return /^[\u0391-\uFFE5\w]+$/.test(value);
		},
		message : '登录名称只允许汉字、英文字母、数字及下划线。'
	},
	safepass : {
		validator : function(value, param) {
			return safePassword(value);
		},
		message : '密码由字母和数字组成，至少6位'
	},
	equalTo : {
		validator : function(value, param) {
			return value == $(param[0]).val();
		},
		message : '两次输入的字符不一至'
	},
	number : {
		validator : function(value, param) {
			return /^\d+$/.test(value);
		},
		message : '请输入数字'
	},
	numberzero : {
		validator : function(value, param) {
			return /^[1-9]\d*$/.test(value);
		},
		message : '请输入非零的数字'
	},

	carNo : {
		validator : function(value) {
			return /^[\u4e00-\u9fa5]{1}[A-Z]{1}[A-Z_0-9]{5}$/.test(value);
		},
		message : "车牌号码无效（例：粤J12350）"
	},
	idcard : {
		validator : function(value, param) {
			return idCard(value);
		},
		message : '请输入正确的身份证号码'
	},

	decimal : {
		validator : function(value, param) {
			if(value.indexOf('￥') != -1 || value.indexOf(',') != -1){
			  value = $.unformatMoney(value.substring(1,value.length));
			}
			return /^\d{1,10}(\.\d{1,2})?$/.test(value);
		},
		message : '只能输入数字且整数最多10位，小数点后最多两位'
	},

	weishudecimal : {
		validator : function(value, param) {
			return /^(?!^0\.0?0$)^[1-9][1-9]?(\.[0-9]{1,2})?$|^100$/.test(value);
		},
		message : '只能输入大于0小于100的数字且小数点后最多两位'
	},
	thanzero : {
		validator : function(value, param) {
			return /^[1-9]\d*$/.test(value);
		},
		message : '只能输入大于0的整数'
	},
	zimuzw : {
		validator : function(value, param) {
			return /^[a-zA-Z\u4e00-\u9fa5]+$/.test(value);
		},
		message : '只能输入字母或者中文'
	},
	dianhua : {
		validator : function(value, param) {
			return /^((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/
					.test(value);
		},
		message : '电话格式不正确'
	},
	faxno : {// 验证传真 
        validator : function(value) { 
            return /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i.test(value); 
        }, 
        message : '传真号码不正确' 
    }, 
	haoma : {
		validator : function(value, param) {
			return /^[^\u4e00-\u9fa5]{0,}$/.test(value);
		},
		message : '不能输入中文'
	},
	yhk : {
		validator : function(value, param) {
			return /^\d{19}$/.test(value);
		},
		message : '银行卡格式不对'
	},
	letterOrNumber : {
		validator : function(value, param) {
			return /^[A-Za-z0-9]+$/.test(value);
		},
		message : '只能是字母或者数字'
	},
	letterOrNumberOrChinese : {
		validator : function(value, param) {
			return /^[A-Za-z0-9_\u4e00-\u9fa5]+$/.test(value);
		},
		message : '只能是字母或者数字和中文'
	},
	uniqueness : {
		validator : function(value, param) {
			return uniqueness(value, param);
		},
		message : '输入的车牌号重复'
	},
	acturalsize : {
		validator : function(value, param) {
			var beginDate = "";
			var endDate = "";
			if (param[1] == '1') {
				beginDate = value;
				endDate = $(param[0]).datebox('getValue');
			}
			if (param[1] == '2') {
				beginDate = $(param[0]).datebox('getValue');
				endDate = value;
			}
			return actural(beginDate, endDate);
		},
		message : '开始日期不能大于结束日期,请重新选择'
	},
	amountmoney : {
	  validator : function(value, param) {
	     	var lastValue = value.substring(1,value.length);
			return parseFloat($.unformatMoney(lastValue)) <= parseFloat($(param[0]).val()) ;
		},
		message : '批准金额不能大于申请金额'
	},
	per : {
		validator : function(value, param) {
			return /^\d{1,2}(\.\d{1,2})?$/.test(value);
		},
		message : '小数点前最多两位，小数点后最多两位'
	},
	perMax : {
		validator : function(value, param) {
			var sign1 = /^\d{1,3}(\.\d{1,2})?$/.test(value);
			var sign2 = value <= 100;
			var sign = false;
			if(sign1 && sign2){
				sign = true;
			}
			return sign;
		},
		message : '只可填写不大于100内的数字且小数点后最多两位'
	},
	phoneCheck : {
	  validator : function(value, param) {
	     	
			return value != $(param[0]).val() ;
		},
		message : '联系电话1和联系电话2不能够相同'
	},
	radio: {
        validator: function (value, param) {
            var frm = param[0], groupname = param[1], ok = false;
            //查找表单中所有此名称的radio
            $('input[name="' + groupname + '"]', document[frm]).each(function () {
                if (this.checked) { ok = true; return false; }
            });

            return ok
        },
        message: '需要选择一项！'
    },
    checkbox: {
        validator: function (value, param) {
            var frm = param[0], groupname = param[1], checkNum = 0;
            //查找表单中所有此名称的checkbox
            $('input[name="' + groupname + '"]', document[frm]).each(function () {
                if (this.checked) checkNum++;
            });
            return checkNum > 0;
        },
        message: '请选择其中一项或多项！'
    },
    checkDateA :{
    	validator: function (value, param) {
           return /^((?:19|20)\d\d)-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])$/.test(value);
        },
        message: '请选择正确的日期,格式:2015-01-01'
      
    }
});

/*
 * easyui radio checkbox 验证
 * o 需要验证的radio、checkbox对象数组的第一个
 * */
function check(o) {
    if (!o.validatebox('isValid')) {
    	o.parent().addClass('validatebox-invalid');
    } else {
    	o.parent().removeClass('validatebox-invalid');
    }
    return false;
}

/**
 * 判断开始日期是否大于结束日期
 * @param {} value
 * @param {} param
 * @return {Boolean}
 */
function actural(beginDate, endDate) {
	if (beginDate != "" && endDate != "") {
		var num = Timer.DaysBetween(endDate, beginDate);
		if (num < 0) {
			return false;
		} else {
			return true;
		}
	} else {
		return true;
	}
}

//验证唯一
function uniqueness(value, param) {
	var url = encodeURI(param[0] + value);
	var flag = false;
	$.ajax({
				type : "POST",
				url : url,
				async : false,
				dataType : "text",
				success : function(text) {
					var json = eval('(' + text + ')');
					flag = json.success;
				}
			});
	return flag;
}

/* 密码由字母和数字组成，至少6位 */
var safePassword = function(value) {
	return !(/^(([A-Z]*|[a-z]*|\d*|[-_\~!@#\$%\^&\*\.\(\)\[\]\{\}<>\?\\\/\'\"]*)|.{0,5})$|\s/
			.test(value));
}

var idCard = function(value) {
	if (value.length == 18 && 18 != value.length)
		return false;
	var number = value.toLowerCase();
	var d, sum = 0, v = '10x98765432', w = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7,
			9, 10, 5, 8, 4, 2], a = '11,12,13,14,15,21,22,23,31,32,33,34,35,36,37,41,42,43,44,45,46,50,51,52,53,54,61,62,63,64,65,71,81,82,91';
	var re = number
			.match(/^(\d{2})\d{4}(((\d{2})(\d{2})(\d{2})(\d{3}))|((\d{4})(\d{2})(\d{2})(\d{3}[x\d])))$/);
	if (re == null || a.indexOf(re[1]) < 0)
		return false;
	if (re[2].length == 9) {
		number = number.substr(0, 6) + '19' + number.substr(6);
		d = ['19' + re[4], re[5], re[6]].join('-');
	} else
		d = [re[9], re[10], re[11]].join('-');
	if (!isDateTime.call(d, 'yyyy-MM-dd'))
		return false;
	for (var i = 0; i < 17; i++)
		sum += number.charAt(i) * w[i];
	return (re[2].length == 9 || number.charAt(17) == v.charAt(sum % 11));
}

var isDateTime = function(format, reObj) {
	format = format || 'yyyy-MM-dd';
	var input = this, o = {}, d = new Date();
	var f1 = format.split(/[^a-z]+/gi), f2 = input.split(/\D+/g), f3 = format
			.split(/[a-z]+/gi), f4 = input.split(/\d+/g);
	var len = f1.length, len1 = f3.length;
	if (len != f2.length || len1 != f4.length)
		return false;
	for (var i = 0; i < len1; i++)
		if (f3[i] != f4[i])
			return false;
	for (var i = 0; i < len; i++)
		o[f1[i]] = f2[i];
	o.yyyy = s(o.yyyy, o.yy, d.getFullYear(), 9999, 4);
	o.MM = s(o.MM, o.M, d.getMonth() + 1, 12);
	o.dd = s(o.dd, o.d, d.getDate(), 31);
	o.hh = s(o.hh, o.h, d.getHours(), 24);
	o.mm = s(o.mm, o.m, d.getMinutes());
	o.ss = s(o.ss, o.s, d.getSeconds());
	o.ms = s(o.ms, o.ms, d.getMilliseconds(), 999, 3);
	if (o.yyyy + o.MM + o.dd + o.hh + o.mm + o.ss + o.ms < 0)
		return false;
	if (o.yyyy < 100)
		o.yyyy += (o.yyyy > 30 ? 1900 : 2000);
	d = new Date(o.yyyy, o.MM - 1, o.dd, o.hh, o.mm, o.ss, o.ms);
	var reVal = d.getFullYear() == o.yyyy && d.getMonth() + 1 == o.MM
			&& d.getDate() == o.dd && d.getHours() == o.hh
			&& d.getMinutes() == o.mm && d.getSeconds() == o.ss
			&& d.getMilliseconds() == o.ms;
	return reVal && reObj ? d : reVal;
	function s(s1, s2, s3, s4, s5) {
		s4 = s4 || 60, s5 = s5 || 2;
		var reVal = s3;
		if (s1 != undefined && s1 != '' || !isNaN(s1))
			reVal = s1 * 1;
		if (s2 != undefined && s2 != '' && !isNaN(s2))
			reVal = s2 * 1;
		return (reVal == s1 && s1.length != s5 || reVal > s4) ? -10000 : reVal;
	}
};

/** 金额转换 */
function setNewValue(valueId){
    var oldValue = $('#'+valueId).val();
	if(oldValue != "" && oldValue.indexOf('￥') != -1){
      var lastValue = oldValue.substring(1,oldValue.length);
	  $('#'+valueId).val($.unformatMoney(lastValue))
	}
}